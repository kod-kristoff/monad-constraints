use std::rc::Rc;
use persi_ds::unsync::list::{concat_all, fmap, List};
...
pub fn eval_state_list<State: Clone, A: Copy>(st: &StateList<State, A>, s: &State) -> List<A> {
...
pub fn mzero<'a, State, A>() -> StateList<'a, State, A> {
    Rc::new(move |_s: &State| PairList::new())
}
